...
unsigned char Encrypt_Des(unsigned char *In, unsigned char *Out, unsigned char *Key, unsigned char Type)
{
	deskey(Key, Type);
	des(In, Out);
...
unsigned char Encrypt_3Des(unsigned char *In, unsigned char *Out, unsigned char *Key, unsigned char Type)
{
	desDkey(Key, Type);
	Ddes(In, Out);
...
void Encrypt_3Des_Group(unsigned char *dSourceData, unsigned char *dTargetData, unsigned char *dKey, unsigned int group_num, unsigned char type)
{
	unsigned int i=0;
	for (i=0; i<group_num; i++)
		Encrypt_3Des(&dSourceData[i*8], &dTargetData[i*8], dKey, type);
}
